...
import '../service/asm_business_service.dart';
...
class _BusinessPageState extends State<BusinessPage> {
  ...
  Future<void> _loadVisitors() async {
    ...
    try {
      final response = await VisitorService.fetchAllVisitors();
      setState(() {
  ...
  void _applyFilters() {
    List<VisitorModel> result = _visitors;

    if (_activeFilter == 'Doctor') {
      result = result.where((v) => v.visitFor == 'Doctor').toList();
    } else if (_activeFilter == 'Business') {
      result = result.where((v) => v.visitFor != 'Doctor').toList();
    } else if (_activeFilter == 'revisit') {
      result = result
  ...
  void _showOrderDialog(VisitorModel visitor) {
    showDialog(
      context: context,
  ...
  void _showVisitDialog(VisitorModel visitor) {
    showDialog(
      context: context,
      barrierDismissible: true,
      builder: (ctx) => Dialog(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                _filterChip('all', 'All'),
                _filterChip('Doctor', 'Doctor'),
                _filterChip('Business', 'Business'),
            ...
            children: [
              Expanded(
                child: _actionButton(
                  ...
                  const Color(0xFF534AB7),
                      () {
                    // TODO: navigate to order details for this visitor
                  },
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: _actionButton(
                  ...
                  ThemeConstants.primaryGreen,
                      () {
                    // TODO: launch dialer, e.g. using url_launcher
                    // launchUrl(Uri(scheme: 'tel', path: visitor.mobile));
                  },
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: _actionButton(
                  ...
                  const Color(0xFF854F0B),
                      () {
                    // TODO: navigate to visit / map screen
                  },
                ),
              ),
            ],
