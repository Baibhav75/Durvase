...
class _RetailerLoginScreenState extends State<RetailerLoginScreen> {
  ...
  Future<void> _loadSavedCredentials() async {
    final credentials = await RetailerSessionManager.getSavedCredentials();
    if (!mounted) return;
  ...
  Future<void> _handleSignIn() async {
    ...
    final result = await RetailerService.login(
      mobile: mobile,
      password: password,
    );
    ...
    if (result.success) {
      if (_rememberMe) {
        await RetailerSessionManager.saveCredentials(mobile, password);
      } else {
        await RetailerSessionManager.clearCredentials();
      }

      if (!mounted) return;
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => const RetailerDashboardPage()),
      );
    } else {
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Text(
                                'Sign in to access your orders, stock & deals',
                                style: GoogleFonts.poppins(
                                  fontSize: 13,
