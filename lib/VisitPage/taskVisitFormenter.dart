...
import '../model/visit_history_model.dart'; // For Visitors model

class NewVisitFormr extends StatefulWidget {
  final Data1? employeeData;
  final Visitors? prefillData;

  const NewVisitFormr({Key? key, this.employeeData, this.prefillData}) : super(key: key);

  @override
  _NewVisitFormState createState() => _NewVisitFormState();
}

class _NewVisitFormState extends State<NewVisitFormr> {
  ...
  void _prefillDataIfAvailable() {
    if (widget.prefillData != null) {
      final visit = widget.prefillData!;
      if (visit.businessName != null) businessNameController.text = visit.businessName!;
      if (visit.personName != null) personNameController.text = visit.personName!;
      if (visit.mobile != null) mobileController.text = visit.mobile!;
      if (visit.address != null) addressController.text = visit.address!;
      if (visit.purpose != null && purposes.contains(visit.purpose)) {
        selectedPurpose = visit.purpose;
      }
      if (visit.visitFor != null && visit.visitFor!.isNotEmpty) {
        visitType = visit.visitFor!;
      }
    }
  }
